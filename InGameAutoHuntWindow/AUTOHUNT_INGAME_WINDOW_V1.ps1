...
$Host.UI.RawUI.WindowTitle = "AUTOHUNT INGAME WINDOW V1"
...
function XmlBytes949($doc){
    ...
    $s.Encoding=[Text.Encoding]::GetEncoding(949)
...
foreach($old in @($mainDoc.SelectNodes("//*[@Name='AutoHuntSettingsWindow']"))){
...
SetAttr $clone "Name" "AutoHuntSettingsWindow"
...
$backup=Join-Path $ClientDir ("AUTOHUNT_INGAME_BACKUP_"+$stamp)
...
"Separate top-level window: AutoHuntSettingsWindow",
"Stored in MainButtonUI.xml so it appears in the game world without opening Character Ranking.",
"CP949 output used for Korean captions.",
"Stage: layout/open-by-default test only.",
"Backup="+$backup
)|Set-Content -Encoding ASCII (Join-Path $ClientDir "AUTOHUNT_INGAME_V1_RESULT.txt")

Ok "PATCH COMPLETE"
Ok "Enter the game world. Do NOT open Character Ranking."
Ok "The separate AutoHuntSettingsWindow should be visible immediately."
